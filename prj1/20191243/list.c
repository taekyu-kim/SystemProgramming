...
void list_swap(struct list_elem *elem1, struct list_elem *elem2){
  ...
  if (elem1->next != elem2){  // elem1과 elem2이 떨어져 있는 경우
...
unsigned long long get_current_microseconds() {
    struct timeval tv;
...
void list_shuffle(struct list *list){
  ...
  unsigned long long seed = get_current_microseconds(); // 현재 시간을 마이크로초 단위로 표현
  srand(seed);
...
bool list_less(const struct list_elem *a, const struct list_elem *b, void *aux){
  return (list_entry(a, struct list_item, elem)->data < list_entry(b, struct list_item, elem)->data) ? true : false;
}